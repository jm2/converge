...
func (a *App) Extensions() []Item {
	return []Item{
		...
		{"Template", "Render Go text/template to a file with variable substitution"},
		{"Hostname", "Ensure system hostname matches desired value"},
		{"Cron", "Manage cron jobs (Linux/macOS) and scheduled tasks (Windows)"},
		{"Repository", "Manage package repository sources (APT, DNF/YUM)"},
		{"KernelModule", "Load or blacklist kernel modules (Linux only)"},
		{"Reboot", "Schedule and track platform-native OS reboots"},
